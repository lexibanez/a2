...
is raised when attempting to deserialize a dsu
 file to a Profile object.
...
class Post(dict):
    ...
    individual user posts. It currently supports
    two features: A timestamp property that is set
    upon instantiation and when the entry object is
    set and an entry property that stores the post message.
    ...
    set capability for entry and time values. When
    the value for entry is changed, or set, the
...
class Profile:
    ...
    this class to manage the information provided by each new user
    created within your program for a2. Pay close attention
    to the properties and functions in this class as you will
    need to make use of each of them in your program.

    When creating your program you will need to collect
    user input for the properties exposed by this class. A
    Profile class should ensure that a username and password
    are set, but contains no conventions to do so. You should
    make sure that your code verifies that required properties are set.

    """

    def __init__(self, username=None, password=None, bio=None, dsuserver=None):
    ...
    def __str__(self):
        return (f'Username: {self.username}, '
                f'Password: {self.password}, Bio: {self.bio}')
    ...
    own search operation on the posts returned from the get_posts
    ...
    def load_profile(self, path: str) -> None:
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                f = open(p, 'r')
                obj = json.load(f)
                if not all(key in obj for key in ['username', 'password',
                                                  'dsuserver',
                                                  'bio', '_posts']):
